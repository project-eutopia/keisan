...
RSpec.describe SymbolicMath::Variables::Registry do
  let(:variables) { {} }
  let(:parent) { nil }
  let(:use_defaults) { true }
  let(:registry) { described_class.new(variables, parent, use_defaults) }

  context "with no parent, and using defaults" do
    it "raises error when not present" do
      expect{registry["not_exist"]}.to raise_error(SymbolicMath::Exceptions::UndefinedVariableError)
    end
  ...
  context "when not using defaults" do
    ...
    it "raises error when getting a default variable" do
      expect{registry["pi"]}.to raise_error(SymbolicMath::Exceptions::UndefinedVariableError)
    end
  end
