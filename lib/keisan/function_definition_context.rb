module Keisan
  class FunctionDefinitionContext < Context
    def initialize(parent:, arguments:, shadowed: [], allow_recursive: false, arguments_context: nil)
      super(parent: parent, shadowed: shadowed + arguments, allow_recursive: allow_recursive)
    ...
    def variable(name)
      @arguments.include?(name) ? @arguments_context.variable(name) : super
    end

    def has_variable?(name)
      @arguments.include?(name) ? @arguments_context.has_variable?(name) : super
    end

    def register_variable!(name, value)
      @arguments.include?(name) ? @arguments_context.register_variable!(name, value) : super
    end
